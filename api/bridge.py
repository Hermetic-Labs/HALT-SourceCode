"""
Universal Translation Bridge — standalone CTranslate2 + espeak-ng service.

Runs as its own FastAPI process (port 7779) to keep heavy NLLB translation
models out of the main API's memory footprint. The main API delegates all
translation work here via WebSocket.

Capabilities:
  - translate  : NLLB-200 CTranslate2 translation between 40+ languages.
  - phonemize  : espeak-ng IPA conversion for Kokoro TTS voice synthesis.

The NLLB model lazy-loads on first request to keep cold-start fast.
"""
...
_MODELS_DIR = (
    ...
    if not __import__("os").environ.get("EVE_MODELS_DIR")
    else Path(__import__("os").environ["EVE_MODELS_DIR"])
)
        ...
        while True:
            ...
            if action == "phonemize":
                ...
                await websocket.send_text(
                    json.dumps({"id": msg_id, "action": action, "text": phonetics, "original": text, "source": source})
                )
                continue
